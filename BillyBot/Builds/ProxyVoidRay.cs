...
{
    private bool CancelledProxyChatSent;
    ...
    private readonly SharkyUnitData SharkyUnitData;
        ...
        : base(defaultSharkyBot, counterTransitioner)
    {
        ...
        ProxyTask = new(SharkyUnitData, false, 0.9f, MacroData, string.Empty, MicroTaskData, defaultSharkyBot.DebugService, defaultSharkyBot.ActiveUnitData, probeMicroController);
    ...
    public override void StartBuild(int frame)
    {
        ...
        defenseSquadTask.DesiredUnitsClaims = new() {new(UnitTypes.PROTOSS_STALKER, 1)};
        ...
        MacroData.Proxies[ProxyTask.ProxyName] = new(proxyLocation, MacroData);
    ...
    private void SetAttack()
    {
        ...
        else if (UnitCountService.Completed(UnitTypes.PROTOSS_VOIDRAY) == 0) AttackData.Attacking = false;
    }
    ...
    public override void OnFrame(ResponseObservation observation)
    {
        ...
        {
            if (MacroData.DesiredPylons < 2) MacroData.DesiredPylons = 2;
            if (ChronoData.ChronodUnits.Contains(UnitTypes.PROTOSS_PROBE)) ChronoData.ChronodUnits.Remove(UnitTypes.PROTOSS_PROBE);
        }

        if (UnitCountService.Completed(UnitTypes.PROTOSS_GATEWAY) > 0)
        {
            if (MacroData.DesiredTechCounts[UnitTypes.PROTOSS_CYBERNETICSCORE] < 1) MacroData.DesiredTechCounts[UnitTypes.PROTOSS_CYBERNETICSCORE] = 1;
            ProxyTask.Enable();
        ...
        {
            if (MacroData.DesiredUnitCounts[UnitTypes.PROTOSS_STALKER] < 1) MacroData.DesiredUnitCounts[UnitTypes.PROTOSS_STALKER] = 1;

            if (MacroData.Proxies[ProxyTask.ProxyName].DesiredProductionCounts[UnitTypes.PROTOSS_STARGATE] < 2) MacroData.Proxies[ProxyTask.ProxyName].DesiredProductionCounts[UnitTypes.PROTOSS_STARGATE] = 2;
        }
        ...
        if (UnitCountService.Count(UnitTypes.PROTOSS_STARGATE) > 0)
        {
            ...
            if (MacroData.Proxies[ProxyTask.ProxyName].DesiredDefensiveBuildingsCounts[UnitTypes.PROTOSS_SHIELDBATTERY] < 2) MacroData.Proxies[ProxyTask.ProxyName].DesiredDefensiveBuildingsCounts[UnitTypes.PROTOSS_SHIELDBATTERY] = 2;
            ...
            {
                if (MacroData.DesiredUnitCounts[UnitTypes.PROTOSS_ZEALOT] < 3) MacroData.DesiredUnitCounts[UnitTypes.PROTOSS_STALKER] = 3;
        ...
        if (MacroData.Frame > SharkyOptions.FramesPerSecond * 4 * 60) ProxyTask.Disable();
    }

    public override bool Transition(int frame)
    {
        if (UnitCountService.Count(UnitTypes.PROTOSS_STARGATE) < 1)
            if (ActiveUnitData.EnemyUnits.Any(e => Vector2.DistanceSquared(new(e.Value.Unit.Pos.X, e.Value.Unit.Pos.Y), new(MacroData.Proxies[ProxyTask.ProxyName].Location.X, MacroData.Proxies[ProxyTask.ProxyName].Location.Y)) < 100))
                return true;

        if (MacroData.Frame > SharkyOptions.FramesPerSecond * 8 * 60) return true;
