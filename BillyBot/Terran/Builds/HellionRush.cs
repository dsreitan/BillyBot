...
public class HellionRush : TerranSharkyBuild
{
    ...
    public override void StartBuild(int frame)
    {
        ...
        BuildOptions.WallOffType = WallOffType.Full;
    ...
    private void Opening(int frame)
    {
        ...
        {
            if (MacroData.DesiredSupplyDepots < 1) MacroData.DesiredSupplyDepots = 1;
        }
        ...
        if (UnitCountService.EquivalentTypeCount(UnitTypes.TERRAN_BARRACKS) > 0) BuildOptions.StrictGasCount = false;

        if (UnitCountService.EquivalentTypeCount(UnitTypes.TERRAN_REFINERY) >= 2)
        {
            if (MacroData.DesiredSupplyDepots < 2) MacroData.DesiredSupplyDepots = 2;
        }

        if (UnitCountService.EquivalentTypeCompleted(UnitTypes.TERRAN_BARRACKS) > 0)
        {
            if (MacroData.DesiredProductionCounts[UnitTypes.TERRAN_FACTORY] < 2) MacroData.DesiredProductionCounts[UnitTypes.TERRAN_FACTORY] = 2;
            ...
            if (MacroData.DesiredUnitCounts[UnitTypes.TERRAN_MARINE] < 2) MacroData.DesiredUnitCounts[UnitTypes.TERRAN_MARINE] = 2;
        }
    }
    ...
    private void Factories()
    {
        if (UnitCountService.EquivalentTypeCount(UnitTypes.TERRAN_FACTORY) > 0)
        {
            ...
            {
                if (MacroData.DesiredAddOnCounts[UnitTypes.TERRAN_FACTORYTECHLAB] < 1) MacroData.DesiredAddOnCounts[UnitTypes.TERRAN_FACTORYTECHLAB] = 1;

                if (UnitCountService.EquivalentTypeCompleted(UnitTypes.TERRAN_FACTORYTECHLAB) > 0) MacroData.DesiredUpgrades[Upgrades.HIGHCAPACITYBARRELS] = true;

                if (UnitCountService.EquivalentTypeCompleted(UnitTypes.TERRAN_FACTORY) >= 2)
                {
                    if (MacroData.DesiredAddOnCounts[UnitTypes.TERRAN_FACTORYREACTOR] < 1) MacroData.DesiredAddOnCounts[UnitTypes.TERRAN_FACTORYREACTOR] = 1;
                }
                ...
                {
                    if (MacroData.DesiredUnitCounts[UnitTypes.TERRAN_HELLION] < 50) MacroData.DesiredUnitCounts[UnitTypes.TERRAN_HELLION] = 50;
                }
            }
        }
    }
    ...
    private void AddProduction()
    {
        if (UnitCountService.EquivalentTypeCount(UnitTypes.TERRAN_COMMANDCENTER) > 1)
        {
            ...
            {
                if (MacroData.DesiredProductionCounts[UnitTypes.TERRAN_FACTORY] <= UnitCountService.Count(UnitTypes.TERRAN_FACTORY)) MacroData.DesiredProductionCounts[UnitTypes.TERRAN_FACTORY]++;
            }
