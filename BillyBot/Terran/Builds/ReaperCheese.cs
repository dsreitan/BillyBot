...
public class ReaperCheese : TerranSharkyBuild
{
    ...
    public ReaperCheese(DefaultSharkyBot defaultSharkyBot, IIndividualMicroController scvMicroController) : base(defaultSharkyBot)
    {
        ...
        ProxyTask = new(defaultSharkyBot.SharkyUnitData, false, 0.9f, MacroData, string.Empty, defaultSharkyBot.MicroTaskData, defaultSharkyBot.DebugService, defaultSharkyBot.ActiveUnitData, scvMicroController);
    ...
    public override void StartBuild(int frame)
    {
        ...
        defenseSquadTask.DesiredUnitsClaims = new() {new(UnitTypes.TERRAN_REAPER, 1)};
    ...
    public override void OnFrame(ResponseObservation observation)
    {
        ...
        {
            if (MacroData.DesiredGases < 1) MacroData.DesiredGases = 1;

            if (MacroData.DesiredProductionCounts[UnitTypes.TERRAN_BARRACKS] < 3) MacroData.DesiredProductionCounts[UnitTypes.TERRAN_BARRACKS] = 3;
        }

        if (UnitCountService.Completed(UnitTypes.TERRAN_BARRACKS) > 0)
        {
            if (MacroData.DesiredUnitCounts[UnitTypes.TERRAN_REAPER] < 10) MacroData.DesiredUnitCounts[UnitTypes.TERRAN_REAPER] = 10;
    ...
    {
        if (UnitCountService.EquivalentTypeCount(UnitTypes.TERRAN_COMMANDCENTER) >= 2) return true;
